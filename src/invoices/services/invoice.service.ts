...
export class InvoiceService {
  ...
  async find(
    queryStringInvoiceDto: QueryStringInvoiceDto,
    ...
    const invoicesNormalized = await this._normalizeCurrencies(
      invoices,
      currency,
    );
  ...
  private async _normalizeCurrencies(
    invoices: InvoiceEntity[],
    qsCurrency: string = null,
  ): Promise<InvoiceEntity[]> {
    ...
    for (const invoice of _invoices) {
      ...
      const newTotal = await this._convertCurrency(
        currencyToUse,
        vendorCurrency,
        total,
      );
      const newPaymentTotal = await this._convertCurrency(
        currencyToUse,
        vendorCurrency,
        paymentTotal,
      );
      const newCreditTotal = await this._convertCurrency(
        currencyToUse,
        vendorCurrency,
        creditTotal,
      );
  ...
  private async _convertCurrency(from: string, to: string, value: number) {
    ...
    try {
      const response = await lastValueFrom(
        this._invoiceExternalService.convertCurrency<{
